...
const statusConfig = {
  pending: {
    ...
    actions: [
      ...
      { status: "cancelled", label: "Cancel Order", color: "bg-red-100 text-red-800" }
    ]
  },
  shipping: {
    ...
    actions: [
      { status: "completed", label: "Mark as Completed", color: "bg-green-100 text-green-800" },
      { status: "cancelled", label: "Cancel Order", color: "bg-red-100 text-red-800" }
    ]
  },
  completed: {
                    ...
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex space-x-2">
                          {statusConfig[order.order_status]?.actions?.map((action) => (
                            <button
                              key={action.status}
                            ...
                            >
                              {action.status === "complet" && <FiCheck className="mr-1" />}
                              {action.status === "cancel" && <FiX className="mr-1" />}
                              {action.status === "shipping" && <FiTruck className="mr-1" />}
                              {action.label}
                            </button>
                          ))}
                          {!statusConfig[order.order_status]?.actions?.length && (
