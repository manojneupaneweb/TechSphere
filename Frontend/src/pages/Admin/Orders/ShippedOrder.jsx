...
const statusConfig = {
  pending: {
    ...
    actions: [
      { status: "shipping", label: "Mark as Shipping", color: "bg-blue-100 text-blue-800" },
      { status: "cancel", label: "Cancel Order", color: "bg-red-100 text-red-800" }
    ]
  },
  shipping: {
    ...
    actions: [
      { status: "complete", label: "Mark as Completed", color: "bg-green-100 text-green-800" },
      { status: "cancel", label: "Cancel Order", color: "bg-red-100 text-red-800" }
    ]
  },
  completed: {
...
function ConfirmationModal({ open, onClose, onConfirm, status, processing }) {
  if (!open) return null;
  return (
  ...
  return (
    ...
    >
      ...
      <td className="px-6 py-4 whitespace-nowrap">
        <div className="flex items-center">
          <span
            className={`px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${config.color || "bg-gray-100 text-gray-800"}`}
          >
            {config.icon}
        ...
        <div className="flex space-x-2">
          {config.actions?.length > 0 ? (
            config.actions.map((action) => (
              ...
              >
                {action.status === "completed" && <FiCheck className="mr-1" />}
                {action.status === "cancelled" && <FiX className="mr-1" />}
                {action.label}
              </button>
            ))
          ) : (
...
const OrderManagement = () => {
  ...
  // Only show orders with status "shipping"
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredOrders.length === 0 ? (
                  ...
                  filteredOrders.map((order) => (
                    <OrderRow key={order.id} order={order} onAction={showConfirmation} />
                  ))
                )}
              </tbody>
            </table>
